...
class ETGuildQueue {
     ...
     * @param {string} [fill=''] - The character used for the filled part of the progress bar (if provided).
     *
     * @returns {TrackProgress} An object representing the track's progress.
     */
    progress(size = 20, pointer = '●', line = '▬', fill = '') {
        const track = this.tracks[this._trackIndex];
        const isFillBar = typeof fill === 'string' && fill.length > 0;
        if (!track ||
            track.duration.value <= 0 ||
            !this.connection.current?.startTime) {
            return {
                bar: line.repeat(Math.max(0, size)),
                percents: 0,
            };
        }
        const total = track.duration.value;
        const elapsed = (Date.now() - this.connection.current.startTime) / 1000;
        const seek = this._seek ?? 0;
        const current = Math.min(total, Math.max(0, Math.floor(elapsed + seek)));
        const progress = current / total;
        const filledLength = Math.min(size - 1, Math.floor(progress * size));
        const bar = (isFillBar ? fill.repeat(filledLength) : line.repeat(filledLength)) +
            pointer +
            line.repeat(Math.max(0, size - filledLength - 1));
        return {
            bar: bar,
            percents: Math.round(progress * 100),
        };
    }
    /**
