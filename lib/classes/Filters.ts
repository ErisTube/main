// Import types
import { PlaybackFilter } from '../../types';
...
export class ETFilters {
	...
	public register(...data: RestOrArray<PlaybackFilter>): PlaybackFilter[] {
		const filters = normalizeArray(data);
		...
		for (const filter of filters) {
			...
			if (this._debug) {
				console.log(
					`[ErisTube Filters] Filter '${filter.name}' successfully registered!`
				);
			}
		}
	...
	public get(name: string): PlaybackFilter {
		...
		if (this._debug) {
			console.log(
				`[ErisTube Filters] Filter: ${name}, registered: ${Boolean(filter)}`
			);
		}
	...
	public unregister(...data: string[]): PlaybackFilter[] {
		...
		for (const filter of filters) {
			...
			if (this._debug) {
				console.log(
					`[ErisTube Filters] Filter '${filter}' successfully unregistered!`
				);
			}
		}
